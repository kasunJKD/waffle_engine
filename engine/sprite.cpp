...
SpriteManager* sp_mgr
    ...
    }
    {
        ...
        s->frame_size= glm::vec2(32, 32);
        s->sheet_size = glm::ivec2(512, 288);  // Atlas = pixel pos
        s->pixel_offset = glm::vec2(64,0);
        s->frame_count = 1;
        s->frame_duration = 0.1;

        DEBUG_ASSERT(s!=nullptr, "puzzle sprite is nullptr");

        sp_mgr->spriteTable->ht_insert("puzzle", s);
        sp_mgr->count++;
    }
    {
        Sprite* s = (Sprite*)Pool_Allocator::pool_alloc(sp_mgr->pool);
        ...
        DEBUG_ASSERT(s!=nullptr, "puzzle_valid sprite is nullptr");

        sp_mgr->spriteTable->ht_insert("puzzle_valid", s);
        sp_mgr->count++;
    }
