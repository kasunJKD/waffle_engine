...
private:
    ...
    Mesh processMesh(aiMesh* mesh, const aiScene* scene)
    {
        ...
        if (mesh->mMaterialIndex >= 0)
        {
            ...
            // For PBR materials in glTF
            std::vector<Texture> baseColorMaps = loadMaterialTextures(material, aiTextureType_BASE_COLOR, "texture_base_color");
            textures.insert(textures.end(), baseColorMaps.begin(), baseColorMaps.end());
            std::vector<Texture> metallicRoughnessMaps = loadMaterialTextures(material, aiTextureType_METALNESS, "texture_metallic_roughness");
...
inline GLuint TextureFromFile(const char* path, const std::string& directory)
{
